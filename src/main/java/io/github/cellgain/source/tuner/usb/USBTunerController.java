...
public abstract class USBTunerController extends TunerController
{
    ...
    //Troubleshooting libusb bug: https://github.com/DSheirer/sdrapp/issues/1253
            ...
            else if(status == LibUsb.ERROR_BUSY)
            {
                ...
                if(mAnomalousTransfersDetected == USB_BULK_TRANSFER_BUFFER_POOL_SIZE)
                {
                    mLog.warn("USB transfer anomaly detection count exceeded the total number [8] of available " +
                            "tranfers - sdrapp may no longer be streaming data from this tuner and may appear to " +
    ...
    class UsbEventProcessor implements Runnable
    {
        ...
        public void start()
        {
            if(mThread == null)
            {
                ...
                mThread.setName("sdrapp USB tuner - bus [" + mBus + "] port [" + mPortAddress + "]");
